...
Device-specific manifests (IPs, service names) generated by `bridge-phone.sh` are stored in the `mobile-nodes/local-manifests/` directory. This directory is **Git-ignored** to prevent leaking local network information to public repositories. 

To re-provision or update a node's cluster registration, simply re-run `./bridge-phone.sh <IP> <ROLE> <ID>`.
